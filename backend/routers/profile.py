...
from backend.services.minecraft import MinecraftLookupError, resolve_minecraft_profile
...
def serialize_profile(user: User) -> dict[str, Any]:
    return {
        ...
        "minecraft": {
            ...
            "account_type": user.minecraft_account_type,
            "premium": bool(user.minecraft_premium),
            "head_url": user.minecraft_head_url,
            "avatar_url": user.minecraft_avatar_url,
            "linked": bool(user.minecraft_ign),
        },
        "stats": {
...
async def update_minecraft_profile(
    ...
    try:
        profile = await resolve_minecraft_profile(payload.ign, payload.account_type)
