...
EXCLUDED_STATS_STATUSES = {"cancelled", "refunded"}
...
def recalculate_user_order_stats(db: Session, user: User) -> None:
    active_orders = (
        db.query(Order)
        .filter(Order.user_id == user.id, ~Order.status.in_(EXCLUDED_STATS_STATUSES))
        .all()
    )

    user.order_count = len(active_orders)
    user.purchase_count = sum(sum(item.quantity for item in order.items) for order in active_orders)
    user.money_spent_inr = sum(order.final_total for order in active_orders)
...
def create_order_from_cart(db: Session, user: User, cart: Cart, coupon_code: str | None = None) -> Order:
    ...
    recalculate_user_order_stats(db, user)
    clear_cart(db, cart)
    db.commit()
    db.refresh(order)
    return order
...
def update_order_status(db: Session, order: Order, new_status: str, actor: User | None, note: str = "") -> Order:
    ...
    recalculate_user_order_stats(db, order.user)
    db.commit()
    db.refresh(order)
    return order
