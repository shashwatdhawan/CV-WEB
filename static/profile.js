...
function renderProfile(profile) {
  ...
  if (adminNav) adminNav.hidden = !account.is_admin;
  ...
  if (minecraft.linked) {
    ...
    setText("minecraftType", minecraft.premium ? "Premium Account" : "Cracked Account");
    setText("minecraftUuid", minecraft.premium ? minecraft.uuid : "Offline Account");
    minecraftInput.value = minecraft.ign;
    const selected = document.querySelector(`input[name="accountType"][value="${minecraft.account_type || "premium"}"]`);
  ...
  minecraftForm.addEventListener("submit", async event => {
    ...
    try {
      const response = await fetch("/api/profile/minecraft", {
        ...
        body: JSON.stringify({
          ign: minecraftInput.value.trim(),
          account_type: document.querySelector('input[name="accountType"]:checked')?.value || "premium"
        })
      });
